...
September 15 repair continuation supersedes recommendations below: [reliability audit and repair evidence](reliability-audit-2026-09-15.md). Offline suite: 74/74 files; phrasing/planner audit: 21/21 (was 12/21). Explanation framing, inventory paraphrases and personal recall coverage repaired. Pending actions/plans, previous-file pointers and permission requests now belong to their originating conversation; new/resume/reset/delete/shutdown invalidate outgoing approvals, and deferred tasks retain their original scope. Transition tests and real planner/executor tests use mocked operations only. Personality seed unchanged; guidance clarifies durable memory, provisional knowledge storage, readiness and the implemented-but-disabled collector. Local model replies still include false SQLite corrections and inaccurate current-session-only memory claims. These remain blockers, not passes. Comparator remains 51/61 from the earlier audit; no comparator change/retest this repair pass. Next gate: restart Atlas and read back the user's real favorite games in a fresh conversation, then check the new knowledge/inventory phrasing. No SQL or production writes; passive learning remains parked. The reliability section is not complete.

September 14 capability continuation: [capability/system-awareness checkpoint](capability-awareness-checkpoint-2026-09-14.md). A local executable-contract catalogue and reviewed pipeline guide now replace the stale world-model capability summary. Full tool inventory questions use the deterministic catalogue; specific explanations use the guide/model. Supabase dev_state remains dated feature tracking alongside current code/configuration, with a small retrieval budget for capability questions; its outdated rows were not rewritten. TTS exposes a read-only last-health snapshot. The focused app explanation check is next, not SQL. The practical-use transcript's search, knowledge, factual and audio issues are recorded separately, not claimed fixed.
...
# Latest: factual answer boundary and Neuro source repair

See [knowledge-foundation-2026-09-14.md](knowledge-foundation-2026-09-14.md). Three Neuro records remain in Supabase but are `needs_source`; a targeted, source-reviewed SQL repair is prepared and not yet applied. Factual overview abstention, punctuation-aware retrieval and controller regressions are implemented. Passive topic learning has a sourced starter plan, not an active collector. This does not close the remaining semantic evaluation or general hallucination work.
