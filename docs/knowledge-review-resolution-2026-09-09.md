...
Acceptance clears the displaced claim's evidence, verification dates, freshness deadline, topic tags, and confidence. The new value remains unverified (or needs a source); it cannot inherit verification from the old claim or the candidate payload. Original fields remain in the review's before snapshot. Topic regeneration and a conditional undo command are not implemented yet; recovery would require reviewing the audit snapshot and current state, not blindly restoring it.
