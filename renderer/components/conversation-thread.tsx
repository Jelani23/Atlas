...
export function ConversationThread({ messages, typing, focusMessageId }: ConversationThreadProps) {
  ...
  return (
    <div className="conversation-thread-scroll flex min-h-0 w-full flex-1 flex-col gap-3 overflow-y-auto px-4 pb-12 pt-12 themed-scroll">
      {messages.map((m) => (
        <div
          key={m.id}
          ...
          } max-w-[90%] ${m.id === focusMessageId ? "ring-2 ring-primary/40" : ""}`}
        >
          <span className="text-[9px] font-medium uppercase tracking-[0.12em] text-primary/70">
            {m.role === "atlas" ? "Alice" : "You"}
          </span>
          <p className="whitespace-pre-wrap text-pretty text-[14px] leading-[1.5] text-foreground">
            {m.text}
          </p>
        </div>
      ))}
      ...
      {typing && (
        <div className="flex items-center gap-1.5 self-start rounded-2xl border border-border/60 bg-card/70 px-3.5 py-2.5">
