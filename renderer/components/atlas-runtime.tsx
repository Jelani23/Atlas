...
const STATUS_COPY: Record<AtlasConnectionState, { title: string; detail: string }> = {
  connecting: {
    title: "Finding Alice…",
    detail: "Looking for the ATLAS host on your tailnet.",
  },
  initializing: {
    title: "Alice is waking up…",
    detail: "The host is reachable and ATLAS is still starting.",
  },
  online: {
    title: "Alice is here",
    detail: "The ATLAS host is connected and live features are available.",
  },
  offline: {
    title: "Alice is offline",
    detail: "The host is not reachable right now. Your saved Atlas data is still available.",
  },
}
...
function ConnectionIndicator({ state }: { state: AtlasConnectionState }) {
  ...
  if (!expanded) {
    return (
      <button
        ...
        className="alice-edge-tab pointer-events-auto -mr-1 flex h-11 w-11 items-center justify-center text-sky-deep/70"
  ...
  return (
    <div className="alice-surface animate-alice-unfold pointer-events-auto w-[min(90vw,360px)] overflow-hidden">
      ...
      <form
        className="border-t border-white/32 bg-white/18 px-4 py-3.5"
        onSubmit={(event) => {
      ...
      >
        <label htmlFor="atlas-remote-origin" className="font-display text-[13px] text-foreground/55">
          Host address
        </label>
        <div className="mt-2 flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-[1.15rem_1.35rem_1.2rem_1.45rem] border border-white/42 bg-white/46 px-3 py-2 text-xs text-foreground outline-none transition focus:border-white/68 focus:bg-white/66"
          ...
          <button
            type="submit"
            className="alice-nav-item bg-white/72 px-3 py-2 font-display text-[14px] text-sky-deep/80 shadow-sm transition hover:bg-white"
          >
            Connect
          </button>
        </div>
          ...
          <div className="flex items-center gap-3">
            {state === "offline" && (
              <button
                ...
                className="inline-flex items-center gap-1 font-display text-[13px] text-sky-deep/62 hover:text-sky-deep"
              >
