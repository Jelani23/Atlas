  ...
  if (!expanded && state !== "locked") {
    return (
      <button
        ...
        className="pointer-events-auto relative isolate flex h-16 w-11 items-center justify-center text-sky-deep/70 transition-transform hover:translate-x-0.5 active:translate-x-0 active:scale-95"
      ...
      >
        <CloudSkin asset="edgeTab" mirrorX className="-bottom-2 -left-0 -right-7 -top-2 h-[calc(100%+1rem)] w-[calc(100%+1.75rem)] opacity-76" />
        {state === "checking" ? (
          <LoaderCircle className="relative z-10 h-[17px] w-[17px] animate-spin" strokeWidth={1.8} aria-hidden="true" />
        ) : state === "online" ? (
          <Cloud className="relative z-10 h-[18px] w-[18px]" strokeWidth={1.8} aria-hidden="true" />
        ) : (
          <CloudOff className="relative z-10 h-[18px] w-[18px]" strokeWidth={1.8} aria-hidden="true" />
        )}
      </button>
    )
  }
      ...
      <div className="relative z-10 px-6 py-5">
          ...
          >
            <div className="flex gap-2">
              ...
              <button
                ...
                className="alice-nav-item bg-white/58 px-3 py-2 font-display text-[14px] text-sky-deep/80 transition hover:bg-white/78 disabled:cursor-not-allowed disabled:opacity-40"
              >
        ...
        {state === "offline" && (
          <div className="mt-4 flex items-center justify-between gap-3 border-t border-sky-deep/10 pt-3">
            ...
            <button type="button" onClick={() => void refreshAtlasCloudState()} className="font-display text-[13px] text-sky-deep/64 transition hover:text-sky-deep">Try again</button>
          </div>
        )}
